...
export type AccountValue = { name: string; kind: AccountKind; canFundTransfers: boolean; sweepToAccountId: string | null; keepAmount: number }
...
export default function AccountDialog({ account, accounts, onClose, onSave }: { account: AccountRow | null; accounts: AccountRow[]; onClose: () => void; onSave: (value: AccountValue) => void }) {
  ...
  const valid = Boolean(name.trim()) && (!business || Boolean(sweepTo))
  const save = () => {
    if (!valid) return
    onSave({ name: name.trim(), kind, canFundTransfers: business ? false : canFundTransfers, sweepToAccountId: business ? sweepTo : null, keepAmount: business ? keep : 0 })
  }
  return <Dialog title={account ? 'Счёт' : 'Новый счёт'} onClose={onClose} actions={<><Button onClick={onClose}>Отмена</Button><Button variant="primary" disabled={!valid} onClick={save}>Сохранить</Button></>}>
    <form className="form-grid" onSubmit={(event) => { event.preventDefault(); save() }}>
      ...
      {business ? <>
        ...
        <p className="note field-wide">Налоги и бухгалтера добавьте в платежи с этого счёта. Всё, что сверх них и запаса, — одним переводом на выбранный счёт.</p>
