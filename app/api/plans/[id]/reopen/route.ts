...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const result = await changePlanStatus(session, id, 'reopen', parsed.data.expectedVersion)
  return Response.json(result.current ?? { error: 'Not found' }, { status: result.result === 'saved' ? 200 : result.result === 'missing' ? 404 : 409, headers: privateHeaders })
}
