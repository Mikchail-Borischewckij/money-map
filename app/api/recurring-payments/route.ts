...
export async function GET() {
  ...
  return session ? Response.json(await listTemplates(session, 'payment'), { headers: privateHeaders }) : Response.json({ error: 'Unauthorized' }, { status: 401, headers: privateHeaders })
}
export async function POST(request: Request) {
  ...
  const row = await createTemplate(session, 'payment', parsed.data)
  return Response.json(row ?? { error: 'Invalid account or category' }, { status: row ? 201 : 400, headers: privateHeaders })
}
