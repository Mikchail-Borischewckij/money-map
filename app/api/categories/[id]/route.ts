...
// Deletes a category no regular payment uses. Months keep the category name as text, so nothing else changes.
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const rows = await db()`DELETE FROM categories c WHERE c.id = ${id} AND c.household_id = ${session.householdId} AND c.version = ${parsed.data.version}
    ...
    AND NOT EXISTS (SELECT 1 FROM payment_template_versions v WHERE v.category_id = c.id) RETURNING c.id`
